    ...
    share(option: "url" | "twitter" | "telegram" | "whatsapp") {
      if (option === "url") {
        ...
        if (!el) {
          ...
          el.value = this.path ? `https://www.ilhamtaufiq.dev${this.path}` : location.href
        ...
        switch (option) {
          case "twitter":
            url = `https://twitter.com/intent/tweet?via=ilhamtopiq&text=${encodeURIComponent(
              this.title + "\n" + location.href
            )}`
            break
          case "telegram":
  ...
  <div v-else class="grid gap-4 grid-cols-1 sm:grid-cols-3">
    ...
    >
      <input
        ref="share-url"
        readonly
        :value="`https://www.ilhamtaufiq.dev${path}`"
